...
- Treat tag pushes, `npm publish`, `npm unpublish`, and dist-tag changes as
  explicit release actions; do not infer authorization from a review, sync, or
  version bump alone.
...
- Publish in dependency order, with `goatx402-sdk` before
  `goatx402-quickpay`.
