...
<!-- REVISION NOTE: This matrix is mainnet-only. Metis and Tempo are DIRECT-only because DELEGATE requires Permit2 plus an approved callback contract on the merchant chain. Metis explorer is documented with the verified official Blockscout URL, although the current code map still contains explorer.metis.io. -->
...
// Signature algorithm:
// 1. Build signature params from request body/query plus api_key, timestamp, and nonce
...
<!-- REVISION NOTE: Verified against goatx402-core/internal/api/signature.go and both server SDKs: X-Nonce is required and the signed params include `nonce`. -->
...
<!-- REVISION NOTE: EIP-712 fields are verified against goatx402-core/internal/store/facade_order.go and goatx402-sdk/src/types.ts. -->
